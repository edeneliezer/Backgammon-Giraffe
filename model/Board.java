...
public class Board {
    ...
    private int doubleValue,game = 1;
    ...
    //Cheat function places two pieces for each player in the ace position
    public void cheat() {
        for (int player=0; player<Backgammon.NUM_PLAYERS; player++)  {
            for (int pip=0; pip<NUM_SLOTS; pip++)   {
    ...
    //Resets board and doubling cube back to initial positions
    public void reset() {
    	for (int player=0; player<Backgammon.NUM_PLAYERS; player++)  {
            for (int pip=0; pip<NUM_SLOTS; pip++)   {
                checkers[player][pip] = RESET[pip];
    	...
    	players.get(0).resetDouble();
    	players.get(1).resetDouble();
    }
    
    // Set current value for the doubling cube
    public void setDoubleValue(int input){
    	this.doubleValue = input;
    }
    
    // Get value for the doubling cube
    public int getDoubleValue(){
    	return this.doubleValue;
    }

    public int getScore(Player currentPlayer)
    {
